...
$sql = "SELECT *
          FROM Public_Songs
         WHERE Public_Songs.public_song_id = '$song_id';";
$result = $conn->query($sql);
if($result->num_rows == 1) {
    $row = $result->fetch_assoc();

    // Get song_id for Song_Tracks query
    $song_id = $row["public_song_id"];
...
$sql = "SELECT *
          FROM Public_Song_Tracks
         WHERE Public_Song_Tracks.public_song_id = '$song_id'
         ORDER BY Public_Song_Tracks.public_track_id;";
$result = $conn->query($sql);
